...
class MenuItem(models.Model):
    parent = models.ForeignKey('self', verbose_name=gettext_lazy('parent'), null=True, blank=True, on_delete=models.SET_NULL)
    caption = models.CharField(gettext_lazy('caption'), max_length=50)
    url = models.CharField(gettext_lazy('URL'), max_length=200, blank=True)
    named_url = models.CharField(gettext_lazy('named URL'), max_length=200, blank=True)
    level = models.IntegerField(gettext_lazy('level'), default=0, editable=False)
    rank = models.IntegerField(gettext_lazy('rank'), default=0, editable=False)
    menu = models.ForeignKey('Menu', related_name='contained_items', verbose_name=gettext_lazy('menu'), null=True, blank=True, editable=False, on_delete=models.CASCADE)
    ...
    def save(self, force_insert=False, **kwargs):
        from treemenu.utils import clean_ranks
    ...
    def get_flattened(self) -> Iterable:
        flat_structure = [self]
        for child in self.children():
...
class Menu(models.Model):
    ...
    root_item = models.ForeignKey(MenuItem, related_name='is_root_item_of', verbose_name=gettext_lazy('root item'), null=True, blank=True, editable=False, on_delete=models.CASCADE)

    def save(self, force_insert=False, **kwargs):
        if not self.root_item:
            root_item = MenuItem()
            root_item.caption = gettext('root_' + self.name)
    ...
    class Meta:
        verbose_name = gettext('menu')
        verbose_name_plural = gettext('menus')